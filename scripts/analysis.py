...
#torino:
qubits_list=[[88, 87, 94], [29, 28, 36], [12, 13, 18], [46, 47, 55], [61, 54, 60], [33, 32, 37], [10, 9, 11], [80, 81, 92], [50, 49, 51], [44, 35, 43]]
#pittsburgh
#qubits_list=[[85,77,84],[38,29,49],[25,24,26],[76,61,81],[93,79,92],[147,146,148],[45,44,46],[98,91,111],[27,17,28],[122,121,123]]
#fez
#qubits_list=[[1, 0, 2], [14, 13, 15], [21, 20, 22], [34, 33, 35], [61, 60, 62], [94, 93, 95], [104, 103, 105], [121, 120, 122], [141, 140, 142], [154, 153, 155]]
#kingston
qubits_list=[[129, 118, 128], [83, 82, 96], [141, 140, 142], [104, 103, 105],[125, 117, 126], [49, 38, 50], [14, 13, 15], [63, 62, 64], [70, 69,71], [67, 57, 68]]
qubits_list=[[1, 0, 2]]
...
class BellResult:
    ...
    def SumResults(self) -> None:
        # There are 10 qubit grups and 8 steering bits
...
def main():
    ...
    for i in range(n_jobs): 
        pd_result = pd.read_csv(
            "C:/Users/UW/Dropbox/lga/"+ results_path + "/results_tests_" + str(i) + ".csv", index_col=0
        )
   ...
   # inequality_values_mean = []
    ...
    for qubit_set_idx in range(n_qubit_sets):
        ...
        ebaC=(c[0] + c[3] + c[2] + c[1])/(n_shots)-(c[0]**2 + c[3]**2 + c[2]**2 + c[1]**2)/(n_shots)**2
        ...
        eBAC=(c[0] + c[3] + c[2] + c[1])/(n_shots)-(ss[0]**2 + ss[3]**2 + ss[2]**2 + ss[1]**2)/(n_shots)**2
        print(BAC,sqrt(eBAC/n_shots)/(4 * weak_meas_rotation_angle * weak_meas_rotation_angle))
        print("ABC")
        ABC = (ss[4] + ss[7] - ss[6] - ss[5]) / (
            n_shots * weak_meas_rotation_angle * weak_meas_rotation_angle * 4
        )
        eABC=(c[4] + c[7] + c[6] + c[5])/n_shots-(ss[4]**2 + ss[7]**2 + ss[6]**2 + ss[5]**2)/(n_shots)**2
        ...
        inequality_errors_BA.append((bAC+BaC)**2*sqrt(eBAC/n_shots)/(baC*16* weak_meas_rotation_angle * weak_meas_rotation_angle*BAC**2))
        inequality_errors_AB.append((AbC+aBC)**2*sqrt(eABC/n_shots)/(abC*16* weak_meas_rotation_angle * weak_meas_rotation_angle*ABC**2))
        val_abC.append(abC)
        val_baC.append(baC)
    ...
    df = pd.DataFrame(
        columns=["qubits","LGBA", "LGeBA","LGAB", "LGeAB","ABC","eABC","BAC","eBAC",\
                 "AbC","eAbC","bAC","ebAC","BaC","eBaC","aBC","eaBC",\
                 "abC","eabC","baC","ebaC","AB","eAB","BA","eBA",\
                 "Ab","eAb","bA","ebA","Ba","eBa","aB","eaB"],\
        index=indices
    )
    ...
    df.to_csv(f"C:/Users/UW/Dropbox/lga/{results_path}/{results_path}.csv")
